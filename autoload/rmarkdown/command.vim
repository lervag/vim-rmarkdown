...
function! rmarkdown#command#call(bang, args)
  ...
  if type(output_type) == type('')
    let opts_data = matchlist(a:args,
          \ '\(-\s\)\@<=\(.*\)\(\s-\)\@=\(\(.*-\s\)\(.*$\)\)*')
    if opts_data != []
      let render_opts = ', '. substitute(opts_data[2], '\"', '\\\"', 'g')
      let object_opts = opts_data[6]
      if !empty(object_opts)
        let output_type_arg = substitute(output_type . '('.object_opts.')',
              \ '\"', '\\\"', 'g')
      endif
    else
      let render_opts_data = matchstr(a:args, '\(-\s\)\@<=.*')
      if !empty(render_opts_data)
        let render_opts  = ', ' .render_opts_data
      else
        let render_opts = ''
        let object_opts = join(args_data[1:], ' ')
        if !empty(object_opts)
          let output_type_arg = substitute(output_type
                \ . '(' . object_opts . ')',
                \ '\"', '\\\"', 'g')
        endif
      endif
  ...
  if has('clientserver') && !empty(v:servername) && executable(s:exexec)
    if a:bang ==# '!'
      let open_arg = '--open'
    else
      let open_arg = '--noopen'
    endif
    silent exe '!'.s:exexec.' --servername '.v:servername
          \ . ' '. open_arg. ' ' .invocation . '&'
...
function! rmarkdown#command#complete(a, c, p)
